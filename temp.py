    ...
    def train(self, tokenized_dataset):
        ...
        for epoch in range(self.training_args.num_train_epochs):
            ...
            if trainer.is_world_process_zero():
                ...
                # Check if this epoch achieved a better accuracy
        ...
        # Save the best_epoch to a file
        with open(os.path.join(self.save_model_path, "best_epoch.txt"), "w") as f:
            f.write(str(best_epoch))

        print(f"Training finished. Best accuracy achieved at epoch {best_epoch} with accuracy {best_accuracy:.4f}")
        return best_epoch  # Return the best epoch number after training
...
def train():
    ...
    best_epoch = model_trainer.train(tokenized_dataset)
    print('Finished training')

def get_best_model_save_path(model_name, best_epoch):
    model_save_dir = args.model_save_dir
    model_name_new = model_name.split('/')[1]
    model_save_path = f"{model_save_dir}{model_name_new}_best_epoch_{best_epoch}"
    return model_save_path
...
def inference():
    ...
    # Load best_epoch from the file
    with open(os.path.join(get_model_save_path(args.model_name), "best_epoch.txt"), "r") as f:
    ...
    best_checkpoint_path = get_best_model_save_path(args.model_name, best_epoch)
    loaded_model = AutoModelForSeq2SeqLM.from_pretrained(best_checkpoint_path)
